...
translate_word_from_failable(W,[[W,E]|_],E) :- !.                                   % Compare the given foreign and/or English word(s) with the first word-pair in the dictionary; if a match is found, stick with the match.
translate_word_from_failable(W,[_|Ds],E) :- translate_word_from_failable(W,Ds,E).   % Recursion: if the translation is not found in the first word-pair, check the next word-pair in the dictionary.
...
translate_word_from_failable(Lang,L,E) :- Lang=latin,dictionary_latin(Ds),translate_word_from_failable(L,Ds,E).     % Translate a Latin and/or English word using the hand-built Latin dictionary.
translate_word_from_failable(Lang,G,E) :- Lang=german,dictionary_german(Ds),translate_word_from_failable(G,Ds,E).   % Translate a German and/or English word using the hand-built German dictionary.

translate_from_failable(_,[],[]).                                                                                           % Base case: no words of either language.
translate_from_failable(Lang,[W|Ws],[E|Es]) :- translate_word_from_failable(Lang,W,E),translate_from_failable(Lang,Ws,Es).  % Recursion: translate the first given foreign and/or English words(s) with a given dictionary using the translate_word function, and recurse with the remaining list of words and the same dictionary.
